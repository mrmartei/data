    ...
    <div className="space-y-4 md:space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-6">
        <div className="bg-slate-900 p-3 md:p-6 rounded-2xl text-white shadow-lg transition-transform hover:scale-[1.01]">
          ...
          <p className="text-slate-400 text-[9px] md:text-xs font-medium uppercase tracking-wider">{userRole === 'admin' ? 'Total Sales' : 'Total Spend'}</p>
          <h2 className="text-base md:text-2xl font-bold tracking-tight">GH₵{userRole === 'admin' ? transactions.reduce((a,c) => a+c.amount, 0).toFixed(2) : '150.00'}</h2>
        </div>
        ...
        <div className="bg-white p-3 md:p-6 rounded-2xl shadow-sm border border-slate-200 transition-transform hover:scale-[1.01]">
          ...
          <p className="text-slate-500 text-[9px] md:text-xs font-medium uppercase tracking-wider">Activity</p>
          <h2 className="text-base md:text-2xl font-bold text-slate-800 tracking-tight">{transactions.length} Orders</h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
        <div className="lg:col-span-2 bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs md:text-sm font-bold text-slate-800 uppercase tracking-widest">Growth Trend</h3>
          </div>
        ...
        <div className="bg-white p-4 md:p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col">
          <h3 className="text-xs md:text-sm font-bold text-slate-800 mb-4 tracking-widest uppercase">Quick Link</h3>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                <span className="text-sm font-bold">{userRole === 'admin' ? 'Manage System' : 'Buy Data Now'}</span>
              </div>
            </div>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setView('history')} className="text-[10px] text-indigo-600 font-bold hover:underline">All</button>
            </div>
              ...
              {transactions.slice(0, 3).map((tx) => (
                <div key={tx.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[11px] font-bold text-slate-800 line-clamp-1">{tx.recipient}</p>
                      <p className="text-[9px] text-slate-400">{tx.date}</p>
                    </div>
                  </div>
                  <span className="text-[11px] font-bold text-slate-700">₵{tx.amount.toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
