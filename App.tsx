...
const App: React.FC = () => {
  ...
  const handleLogin = (identifier: string, pass: string, role: 'user' | 'admin', name?: string) => {
    ...
    // 2. Signup attempt (always 'user' role)
    if (name) {
      const newUser: User = {
  ...
  const handleDataPurchase = (plan: DataPlan, phone: string) => {
    ...
    alert(`Order for ${phone} is processing.`);
    setView('history');
  };
      ...
      case 'settings':
        return (
          <div className="max-w-xl mx-auto bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-200">
            <h2 className="text-xl font-medium text-slate-800 mb-6">Settings</h2>
            <div className="space-y-4">
              <div className="p-5 bg-slate-50 rounded-2xl flex justify-between items-center">
                ...
                <span className="text-[10px] font-medium text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full uppercase tracking-widest">{currentUser?.role}</span>
              </div>
